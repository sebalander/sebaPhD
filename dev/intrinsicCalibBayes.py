...
#    paraMuest1[i], errorMuestras1[i] = nuevo(paraMuest1[i-1], errorMuestras1[i-1])
#    sampleador.mean = paraMuest1[i]
#    sampleador.cov = np.cov(paraMuest1[i-20:i].T)
#
## ahora actualizo pesando por la probabilidad
#for i in range(300, Nmuestras):
#    paraMuest1[i], errorMuestras1[i] = nuevo(paraMuest1[i-1], errorMuestras1[i-1])
##    probMuestras[i] = np.exp(- errorMuestras[i] / 2)
#
#    sampleador.mean = paraMuest1[i]
#    sampleador.cov = np.cov(paraMuest1[200:i].T)
#
#
...
# %% cargo datos
mucovar = np.load("/home/sebalander/Documents/mucov.npy").all()
print(mucovar)
mu2 = mucovar['mu']
covar2 = mucovar['covar']
...
## %%
#
#np.save("/home/sebalander/Documents/datosMHintrinsic", [paraMuest3, paraMuest2])
#
#mucovar = dict()
#mucovar['mu'] = mu2
#mucovar['covar'] = covar2
#np.save("/home/sebalander/Documents/mucov", mucovar)
#
#paramConcat = np.concatenate([paraMuest2, paraMuest3], axis=0)
#corner.corner(paramConcat)
#
#
## %%
##
