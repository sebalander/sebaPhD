...
for k in range(len(X_test)):
    ...
    pyrIm = pyramid(im_test)
    
    cams = []
    fm   = []
    AcumCams = np.zeros([n_x,n_y,10])    
    
    for im in pyrIm:
        s = im.shape
        fm = m2.predict(im.reshape(1,s[0],s[1],1))[0]
        cam = cv2.resize(np.tensordot(fm.weights,axes=(2,1)),(n_x,n_y))
        cams.append(cam)
        acumCams += cam/len(pyIm)
    
    classScore = acumCams.sum(axis=(0,1))        
    pr = softMax.predict(classScore.reshape(1,1,10)/100*100)
